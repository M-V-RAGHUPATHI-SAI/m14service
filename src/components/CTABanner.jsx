...
export default function CTABanner() {
  const clients = businessConfig.stats[1]

  return (
    <section className="py-20 px-12 lg:px-24" style={{ background: '#1C3F3A', position: 'relative', overflow: 'hidden' }}>
      ...
      >
        ...
        <div>
          <h2 style={{ margin: 0 }}>
            <span style={{ display: 'block', fontSize: 'clamp(32px,4vw,56px)', fontWeight: 300, color: 'white', letterSpacing: '-0.02em', lineHeight: 1.1, fontFamily: 'Inter, sans-serif' }}>
              Ready for a space you
            </span>
            <span style={{ display: 'block', fontSize: 'clamp(32px,4vw,56px)', fontWeight: 300, color: 'white', letterSpacing: '-0.02em', lineHeight: 1.1, fontFamily: 'Inter, sans-serif' }}>
              don&apos;t have to{' '}
              <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 800, fontStyle: 'italic', color: 'white' }}>
                think about.
              </span>
            </span>
          </h2>
          <p style={{ fontWeight: 400, fontSize: '16px', color: 'rgba(255,255,255,0.65)', marginTop: '16px', maxWidth: '28rem', lineHeight: 1.75 }}>
            Join {clients.num}{clients.suffix} happy homes across {businessConfig.location.split(',')[0]}. Same-day booking usually available.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col gap-4 lg:items-stretch lg:min-w-[200px]" style={{ flexShrink: 0 }}>
          <Link
            ...
            style={{ display: 'inline-block', textAlign: 'center', textDecoration: 'none', background: 'white', color: '#1C3F3A', fontWeight: 700, fontSize: '15px', padding: '14px 32px', borderRadius: '9999px', border: '2px solid white', cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.22,1,0.36,1)' }}
            onMouseEnter={e => { e.currentTarget.style.background = '#142e2a'; e.currentTarget.style.color = 'white'; e.currentTarget.style.borderColor = '#142e2a'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 12px 32px rgba(0,0,0,0.3)' }}
            onMouseLeave={e => { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = '#1C3F3A'; e.currentTarget.style.borderColor = 'white'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}
          >
