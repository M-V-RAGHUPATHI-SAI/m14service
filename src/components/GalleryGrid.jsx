          ...
          {categories.map((c) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-colors border ${
                filter === c
                  ? 'bg-[#1C3F3A] text-white border-[#1C3F3A]'
                  : 'bg-white text-text-dark border-[#E9EAE8] hover:border-[#1C3F3A]'
              }`}
            >
              {c}
            </button>
          ...
          <AnimatePresence>
            {visible.map((img) => (
              ...
              >
                ...
                />
                <div className="absolute inset-0 bg-[#1C3F3A]/0 group-hover:bg-[#1C3F3A]/40 transition-colors flex items-end p-5">
