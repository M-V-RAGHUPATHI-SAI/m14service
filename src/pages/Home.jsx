...
function Hero() {
  return (
    <section className="relative min-h-screen overflow-hidden">
      ...
      {/* Green overlay — full coverage top to bottom */}
      <div
        className="absolute inset-0 z-10"
        style={{
          background: `linear-gradient(
            to top,
            rgba(28,63,58,0.97) 0%,
            rgba(28,63,58,0.75) 35%,
            rgba(28,63,58,0.55) 65%,
            rgba(28,63,58,0.40) 100%
          )`,
        }}
      />
      ...
      >
        {/* Headline — 300 white block + 800 italic sage green block */}
        <h1 style={{ lineHeight: 1.08, letterSpacing: '-0.02em', textAlign: 'center', maxWidth: '820px' }}>
          <span style={{ fontSize: 'clamp(40px, 5.5vw, 72px)', fontWeight: 300, fontStyle: 'normal', color: 'white', display: 'block' }}>
            {businessConfig.headlineLight}
          </span>
          <span style={{ fontSize: 'clamp(40px, 5.5vw, 72px)', fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700, fontStyle: 'italic', color: 'white', textShadow: '0 2px 24px rgba(0,0,0,0.4)', display: 'block' }}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-10">
          {/* Primary — warm cream fill, like PureClean accent */}
          <Link
          ...
          <Link to="/services" className="group flex items-center gap-3 cursor-pointer" style={{ textDecoration: 'none' }}>
            <span
              style={{
                width: '48px',
                height: '48px',
            ...
            >
              <span style={{ color: 'white', fontSize: '12px', paddingLeft: '2px' }}>▶</span>
            </span>
            <span style={{ color: 'rgba(255,255,255,0.85)', fontSize: '15px', fontWeight: 500 }}>How we work</span>
          </Link>
        </div>
      </div>

      {/* 5. Avatar cluster — absolute bottom right */}
      <div className="absolute z-20 hidden lg:flex items-center gap-3" style={{ bottom: '40px', right: '60px' }}>
        {/* Overlapping avatars */}
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            ...
            timeoutRef.current = setTimeout(() => setActiveIndex(index), 80)
          }
        },
    ...
    <section className="bg-[var(--color-cream)] py-28 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        ...
        <div>
          {services.map((service, i) => {
            ...
            return (
              <div key={service.name}>
                {/* Separator — fades when row above is active */}
                {i > 0 && (
                  <div style={{ height: '1px', background: '#d1d5db', opacity: dividerOp, transition: 'opacity 0.4s ease' }} />
                )}
                ...
                <motion.div
                  ...
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="cursor-pointer flex items-center gap-6 lg:gap-12 min-h-[160px] lg:min-h-[120px]"
                  style={{
                    background: rowBg,
                    borderRadius: '16px',
                    padding: 'clamp(48px, 5vw, 40px) clamp(20px, 4vw, 32px)',
                    opacity: rowOpacity,
                    transition: 'background 0.7s ease, opacity 0.7s ease, padding 0.7s ease, border-radius 0.7s ease',
                  }}
                >
                  {/* Number */}
          ...
          {/* Final separator */}
          <div style={{ height: '1px', background: '#d1d5db', opacity: activeIndex === services.length - 1 ? 0 : 1, transition: 'opacity 0.4s ease' }} />
        </div>
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(
            to top,
            rgba(28,63,58,0.92) 0%,
            rgba(28,63,58,0.55) 35%,
            rgba(28,63,58,0.10) 65%,
            rgba(28,63,58,0.0) 100%
          )`,
        }}
      />
